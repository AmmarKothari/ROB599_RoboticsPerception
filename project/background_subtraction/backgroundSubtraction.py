...
class objectTracking(object):
	# assume the object starts at some known location
	# try to draw a bounding box around it at each frame
	def __init__(self):
		# , object_only_img
		i = 1
...
class cvObjectTracking(object):
	...
	def __init__(self, bbox = None):
		...
		if self.bbox is not None:
			tracker.init(cv2.imread('Stuff1.png'), self.bbox)
	...
	def trackObject(self, img):
		self.tracker.update(img, self.bbox)
...
if __name__ == '__main__':
	...
	cOT = cvObjectTracking()
	cOT.selectBoundingBox(cv2.imread('Stuff2.png'))
